    ...
    >
      ...
      <div className="space-y-4">
        {activityLogs.map((log) => (
          <div
            key={log.id}
            className="border-l-4 border-[#FD90A7] pl-4 py-2 bg-gray-50 rounded-r-xl"
          >
            <p className="text-sm font-medium text-[#1D2130] font-poppins">
              {log.action}
            </p>
            <p className="text-xs text-[#525560] font-poppins">
              {log.user} {log.details && `(${log.details})`} – {log.timestamp}{" "}
              by {log.performedBy}
            </p>
          </div>
        ))}
